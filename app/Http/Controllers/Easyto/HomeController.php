...
namespace App\Http\Controllers\Easyto;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Http\File;
use Illuminate\Support\Facades\Storage;
use App\Services\Easyto\Basics\WebsiteThemeServices;
use App\Services\Easyto\Basics\NewsServices;
use App\Services\Easyto\Basics\FaqServices;
use App\Services\Easyto\Basics\HistoryServices;
use App\Services\Easyto\Basics\JobServices;
...
class HomeController extends Controller
{
    ...
    {
        $list = NewsServices::getByCondition([], '*', 0, 0, 'n_id DESC');
        foreach ($list as $index => $item) {
            $list[$index] = self::formatArrayObject($item, 'formatToArr');
        }
        return view('easyto.news', [
            'list' => $list
        ]);
    }
        ...
        try {
            ...
            if ($request->isMethod('post') && !empty($input)) {
                ...
                if (empty($insert['n_id'])) {
                    unset($insert['n_id']);
                    $res[] = NewsServices::add($insert);
                } else {
                    $res = NewsServices::updateByCondition($insert, $insert['n_id']);
                }
            }
        } catch (Exception $e) {
            $this->responseJson([], 'fail', $e->getMessage());
    ...
    public function getNewsJson(Request $request, $id)
    {
        $res = [];
        try {
            $res = NewsServices::getByCondition([
                'n_id' => $id
            ]);
            if (empty($res)) {
    ...
    public function delNews(Request $request, $id)
    {
        $res = [];
        try {
            $res = NewsServices::deleteByCondition([
                'n_id' => $id
            ]);
            if (!$res) {
