...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'table2', ELEM = '.layui-table2', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'{{# } else if(item2.radio){ }}'
            ,'<th data-field="{{ item2.field||i2 }}" data-type="radio" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} unresize="true"><div class="layui-table-cell laytable-cell-radio"><input type="radio" name="layTableRadio" lay-skin="primary" lay-filter="layTableAllChooseR" {{# if(item2[d.data.radioName]){ }}checked{{# }; }}></div></th>'            
          ,'{{# } else if(item2.space){ }}'
  ...
  Class.prototype.renderForm = function(type){
    form.render((type || 'checkbox' || 'radio'), 'LAY-table-'+ this.index);
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableRadio"]+', function(){
      ...
      if (radio.attr("lay-filter")=="layTableAllChooseR"){
        //add by pyy:清空单选
        $.each(table.cache[that.key],function(i,d) {
          delete d[table.config.radioName];
        });
        return;
      }      
      //add by pyy:添加单选值
      table.cache[that.key][index][table.config.radioName]=checked;      
  ...
  table.radioStatus = function(id){
    ...
    layui.each(data, function(i, item){
      if(item[table.config.radioName]){
        nums++;
        arr.push(table.clearCacheKey(item));
      }
    });
